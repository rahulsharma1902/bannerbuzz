...
function setMinValues() {
    ...
    const minWidthInput = $('#custom_width');
    const minHeightInput = $('#custom_height');
    const minWidth = parseFloat(minWidthInput.val());
    const minHeight = parseFloat(minHeightInput.val());
    ...
    const minAllowed = minValues[unit] || 1;

    if (minWidth < minAllowed || minHeight < minAllowed) {
        $('.errorShowSize').text(`Invalid value: Width and height must be at least ${minAllowed} ${unit}`);
        minWidthInput.val(minAllowed);
        minHeightInput.val(minAllowed);
    } else {
        $('.errorShowSize').empty();
        customSizeChange();
    }
...
$('.applyBtn').on('click', setMinValues);
